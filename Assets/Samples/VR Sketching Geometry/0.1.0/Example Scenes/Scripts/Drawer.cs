...
namespace VRSketchingGeometryPackage.Samples.ExampleScenes.Scripts
{
    public class Drawer : MonoBehaviour
    {
        ...
        public LineSketchObject drawLineThroughPoints(List<Vector3> points)
        {
            ...
            //Setting the properties of the new LineSketchObject with the given LineBrush
            Invoker.ExecuteCommand(new SetBrushCommand(lineSketchObject, newBrush));
        ...
        [SerializeField] private Image huePreview;    // optional, zeigt reines Hue → S=1,V=1
        [SerializeField] private Image satPreview;    // zeigt S bei aktuellem H,V=1
        ...
        public void setColorPreview()
        {
            ...
            // 3) Saturation-Preview: aktuelles S, V=1
            if (satPreview != null)
                satPreview.color = Color.HSVToRGB(h, s, 1f);

            // 4) Value-Preview: S=1, aktuelles V
            if (valPreview != null)
                valPreview.color = Color.HSVToRGB(h, 1f, v);

            new_color = Color.HSVToRGB(colorSlider.value, SatSlider.value, ValSlider.value);
        }


        public void colorLines(Vector3 worldPoint)
        {
            // alle möglichen Treffer in Deinem Layer abfragen
            ...
            foreach (Collider hit in hits)
            {
                ...
                ChangeLineMaterialColorTo(new_color, line);
        ...
        public void deletePoints(Vector3 worldPoint)
        {
            ...
            foreach (Collider hit in hits)
            {
                ...
                // entferne die alte Linie aus der Liste
                _lines.Remove(line);

                // lösche die Punkte & erzeuge Splits
                line.DeleteControlPoints(worldPoint, thickness, out List<LineSketchObject> newLines);

                // --- 1) Original-Linie: entferne ggf. die "Sphere"-Child ---
                Transform origSphere = line.transform.Find("Sphere");
                if (origSphere != null)
                    Destroy(origSphere.gameObject);

                // --- 2) Splits durchlaufen, Sphere wegwerfen und entscheiden: add oder destroy ---
                foreach (LineSketchObject split in newLines)
                {
                    // auch beim Klon die Sphere rausschmeißen
                    Transform splitSphere = split.transform.Find("Sphere");
                    if (splitSphere != null)
                ...
                // --- 3) Original-Linie aufräumen / wieder aufnehmen ---
                var pts = line.GetControlPoints();
                if (pts == null || pts.Count <= 1)
                {
                    // keine echten Linienreste mehr => ganz weg damit
